...
b3sa3_data, b3sa3_md = ts.get_daily(symbol = ('B3SA3.SAO'))
b3sa3_symbol = ts.get_symbol_search('B3SA3.SAO')

# BUSCANDO DADOS DE PETR4
petr4_data, petr4_md = ts.get_daily(symbol = ('PETR4.SAO'))
...
plt.title('Daily Time Series for the B3SA3 and PETR4 stock (close)')
...
plt.plot([1, 2], label='B3SA3')
plt.legend()

plt.plot([1, 2], label='PETR4')
plt.legend()
